...
def build_llm_prompt(
    ...
    for payment in payment_history[-4:]:
        ...
        payment_lines.append(f"{month}: KES {amount:,} / KES {due_amount:,} ({status})")
...
DPD: {additional.get("dpd", 0)} days | EMI Amount: KES {additional.get("amount", "")}
{ml_section}
...
INCLUDE (examples):
- Payment commitments: "Will pay KES 10,000 by Friday"
...
GOOD: {{"label":"DPD","value":"67 days","highlight":true}}, {{"label":"Overdue","value":"KES 73,800","highlight":true}}, {{"label":"Last Paid","value":"Dec KES 5K","highlight":false}}, {{"label":"EMI","value":"KES 18,450","highlight":false}}, {{"label":"Bounce Charges","value":"KES 1,500","highlight":false}}
...
If the conversation indicates the customer is questioning payments, disputing amounts, or the agent needs payment context — surface payment history in contextual_details as TWO separate items: one for paid months and one for unpaid months (e.g. {{"label":"Paid Months","value":"Jun KES 5K, Aug KES 5K","highlight":false}}, {{"label":"Unpaid Months","value":"Jul ✗, Sep ✗","highlight":true}}).
...
Outs:{loan.get("outstanding", "")} Due:{loan.get("overdue", "")} DPD:{additional.get("dpd", 0)}d EMI:KES {additional.get("amount", "")}{ml_line}
Pay:
{payment_toon}
