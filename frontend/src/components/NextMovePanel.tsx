...
import { ArrowRight, ChevronLeft, ChevronRight, Database } from 'lucide-react';
...
export default function NextMovePanel({ nextMove, contextualDetails = [], history = [], fillHeight }: NextMovePanelProps) {
  ...
  return (
    ...
    >
      ...
      {/* Split panel — Bullets always shown; Context panel only when data exists */}
      <div className={`flex-1 grid ${contextualDetails.length > 0 ? 'grid-cols-2 divide-x divide-gray-200' : 'grid-cols-1'} min-h-0`}>
        ...
        {/* RIGHT: Contextual Details — only rendered when data is available */}
        <AnimatePresence>
